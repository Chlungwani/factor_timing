...
# Plot clustermap
sns.clustermap(corr, row_linkage=Z, col_linkage=Z, cmap="coolwarm", figsize=(18, 18))
plt.suptitle("Correlation Clustermap with Dendrogram", y=1.02, fontsize=16)
plt.show()


# Auto-select one feature per cluster
max_clusters = 5  # <-- change this to control the number of clusters
cluster_labels = fcluster(Z, max_clusters, criterion='maxclust')

# Map features to clusters
feature_cluster_map = dict(zip(corr.columns, cluster_labels))

# Select representative feature per cluster (most correlated with others in cluster)
selected_features = []
...
print("Selected features per cluster:")
...
for f in to_drop:
    print(f)
# Optional: Filter original data
features_economic_data = df[selected_features]
features_economic_data.to_csv("features_economic_data.csv", index=False)